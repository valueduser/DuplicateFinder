...
namespace DuplicateFinder.Models
{
	public class Options
	{
		...
		public Options()
		{
			...
			for (var i = 0; configuration[$"Sources:{i}:Name"] != null; i++)
			{
				Source source = new Source()
				{
					Name = configuration[$"Sources:{i}:Name"] ?? $"Source Name {i} Not Found",
					...
					UncPath = configuration[$"Sources:{i}:Path"] ?? "Path Not Found",
					ServerName = configuration[$"Sources:{i}:ServerName"] ?? "Server Name Not Found",
					ShareName = configuration[$"Sources:{i}:ShareName"] ?? "Share Name Not Found",
				};
			...
			Config.PersistenceConfig = new PersistenceConfig()
			{
				...
				User = configuration[$"PersistenceConfig:Username"] ?? "DB User Not Found",
